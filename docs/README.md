...
2. 이벤트 배지 enum 구현
    - [ ] 이벤트 배지의 이름, 배지를 받을 수 있는 최소 혜택 금액을 가진다.

3. Event enum 구현
    - [ ] 방문 날짜 및 이벤트 기간의 최소(1), 최대(31)
    - [ ] 메뉴 개수의 최소(1), 메뉴 개수의 최대(20)
    - [ ] 이벤트가 적용되는 최소 총 주문 금액(10000)
    - [ ] 크리스마스 디데이 날짜 할인 날짜의 최소,최대(1, 25)
    - [ ] 평일 및 주말 할인 금액 (2023)
    - [ ] 특별 할인 금액(1000)
    - [ ] 증정 이벤트 적용 금액(120000)
...
- [ ] Day interface
    - [ ] 날짜에 대한 유효성 검사하는 기능
    - [ ] 해당 날이 주말인지를 리턴하는 기능
    - [ ] 이벤트 달력에 별이 있는 날인지 리턴하는 기능
- [ ] DecemberEventDay
    - [ ] 날짜에 대한 유효성 검사
        - [ ] 1~31사이의 정수인지?
    - [ ] 해당 날이 12월에서 주말이벤트 날인지(금,토) 리턴하는 기능
    - [ ] 12월 25일인지를 리턴하는 기능
    - [ ] 해당 날짜가 이벤트 일(크리스마스) 와 며칠만큼 차이가 나는지 리턴하는 기능
    - [ ] 해당 날짜가 이벤트 일(크리스마스) 이전의 날인가를 리턴하는 기능
